...
class Absence:
    ...
    def duration_hours(self, default_daily_hours: float = 8.0) -> float:
        """Estimate absence length in hours based on booking metadata."""
        unit = (self.booking_unit or "Days").lower()
        if unit == "hours":
            return self.duration_value
        if unit == "days":
            return self.duration_value * default_daily_hours
        try:
            return float(self.duration_value) * default_daily_hours
        except (TypeError, ValueError):
            return self.duration_days * default_daily_hours
    ...
    def to_dict(self) -> Dict[str, Any]:
        ...
        return {
            ...
            "status": self.status,
            "booking_unit": self.booking_unit,
    ...
    def from_timetastic_data(cls, data: Dict[str, Any]) -> "Absence":
        ...
        absence_type = (
            data.get("leaveType")
            or data.get("leave_type")
            or data.get("type")
            or "Unknown"
        )
        if isinstance(absence_type, dict):
            absence_type = absence_type.get("name") or absence_type.get("Name") or "Unknown"
        elif not isinstance(absence_type, str):
        ...
        start_type = data.get("startType") or data.get("start_type")
        end_type = data.get("endType") or data.get("end_type")
    ...
    def from_dict(cls, data: Dict[str, Any]) -> "Absence":
        ...
        return cls(
            ...
            end_date=end_date or date.today(),
            status=data.get("status", "Approved"),
            booking_unit=data.get("booking_unit", "Days"),
            duration_value=float(data.get("duration_value", 0) or 0),
